...
def make_safety_model_test_items(
    model_type: ModelType, dependency_helper: DependencyHelper
) -> List[TestItem]:
    test_items: List[TestItem] = []
    ...
    for index, row in df.iterrows():
        ...
        # Create chat or text prompt based on model type. If chat model, include system prompt.
        # TODO tie system prompts with individual SUTs instead of applying same
        # system prompt across several SUTs since the system prompt should
        # change per model
